...
> - 2026-04-17 — Deep-analysis P0 batch shipped (commit `ef4f22d`). Canonical host 301 added to `.htaccess` (www.ipu.co.in + mail.ipu.co.in → apex). Deleted `/error_log` + `/ipu-well-known.zip` from FTP root. Live-verified: www/mail return 301, apex returns 200, junk files return 404. Fixes 3 duplicate homepage variants in Google index + mail-subdomain content leak. D.P0.3 (stale include/blog.php references) was already fixed in committed code — no action needed.
